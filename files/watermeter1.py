...
server = config.get('watermeter', 'server')
port = int(config.get('watermeter', 'port'))
...
def getData():
    ...
    instrument = watermetereth.Instrument(server, port, 1, debug=False) # port name, slave address
